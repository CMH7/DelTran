...
export function calculateProfit(transaction: Deltran): number {
  ...
  const totalDeductions = transaction.deductions.reduce((sum, deduction) => sum + deduction.amount, 0);

  return transaction.cxPayAmt - transaction.capital - totalExpenses - totalDeductions;
}
